...
namespace TheSandwichShop.Controllers
{
    ...
    public class AuthenticationController : ControllerBase
    {
        ...
        public AuthenticationController(IRepository repository, UserManager<AppUser> userManager, IUserClaimsPrincipalFactory<AppUser> claimsPrincipalFactory)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (user != null && await _userManager.CheckPasswordAsync(user, model.Password))
                {
                    try
                    {
                        ...
                        var fromEmailAddress = "mayurie108@gmail.com"; // you must add your own provided email
                        ...
                        var employee = await _repository.GetEmployeeAsync(user.Id);
                        var bookedShifts = await _repository.GetEmployeeCurrentShift(employee.EmployeeId.ToString());
                        var loggedInUser = new UserViewModel
                        {
                            userName = model.UserName,
                            employeeId = employee.EmployeeId.ToString(),
                            userId = employee.UserId,
                            hasBookedShift = bookedShifts != null ? true : false,
                        };
        ...
        private async Task SendEmail(string fromEmailAddress, string subject, string message, string toEmailAddress)
        {
            ...
            using (var compiledMessage = new MailMessage(fromAddress, toAddress))
            {
                 ...
                 using (var smtp = new SmtpClient())
                {
                    ...
                    smtp.Credentials = new NetworkCredential("mayurie108@gmail.com", "vujs mpzl mfql pyxk"); // your own provided email and password
